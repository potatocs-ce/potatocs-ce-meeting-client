...
export class DrawingService {
  ...
  start(context, points, tool, sourceCanvas) {
    switch (tool.type) {
      ...
      case 'highlighter':
        ...
        context.fillRect(
          points[0] - tool.width / 2,
          points[1] - tool.width / 2,
          tool.width,
          tool.width
        );
        context.fill();
    ...
    switch (tool.type) {
      ...
      case 'eraser': // 지우개는 cover canvas 초기화 후 처음부터 다시 그림 : eraser marker 표시용도...
        context.clearRect(
          0,
          0,
          context.canvas.width / zoomScale,
          context.canvas.height / zoomScale
        );
        if (len < 3) {
          context.beginPath();
          ...
          this.eraserMarker(
            context,
            [points[2 * (len - 1)], points[2 * (len - 1) + 1]],
            tool.width
          );
          break;
        }
      ...
      case 'rectangle':
        if (len > 3) {
          ...
          context.strokeRect(
            points[0],
            points[1],
            points[2 * (len - 1)] - points[0],
            points[2 * (len - 1) + 1] - points[1]
          );
  ...
  clearThumb(data, thumbCanvas, thumbScale) {
    ...
    thumbCtx.clearRect(
      0,
      0,
      thumbCanvas.width / thumbScale,
      thumbCanvas.height / thumbScale
    );
  }
  ...
  async rxDrawingFunc() {
    ...
    if (
      data.tool.type == 'line' ||
      data.tool.type == 'circle' ||
      data.tool.type == 'rectangle' ||
      data.tool.type == 'roundedRectangle' ||
      data.tool.type == 'text'
    ) {
    ...
    if (
      ...
      this.stop = setInterval(() => {
        ...
        if (i === 2) {
          ...
          const b =
            (data.points[2 * (i - 2) + 1] + data.points[2 * (i - 1) + 1]) / 2;
          context.moveTo(a, b);
        }
