...
export class PresentComponent {
	...
	observer: any;
	ngAfterViewInit() {
		// 컴포넌트의 뷰가 초기화된 후 호출되는 Angular 라이프사이클 메서드.
		// ResizeObserver를 사용하여 DOM 요소 크기 변경을 감지.

		this.observer = new ResizeObserver((entries) => {
			// Angular Zone 안에서 실행하여 변경 사항을 Angular의 변경 감지에 반영.
			this.zone.run(() => {
				// 'present_video' ID를 가진 비디오 DOM 요소를 가져옴.
				const video_target: any = document.getElementById("present_video");

				// 비디오 크기 조정을 수행하는 메서드 호출.
