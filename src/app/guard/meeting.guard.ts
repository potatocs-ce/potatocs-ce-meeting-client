...
export const meetingGuard: CanActivateFn = async (route, state) => {
	...
	try {
		const meetingInfo: any = await lastValueFrom(meetingApiService.getMeetingInfo(route.params["id"])).catch(
			(err) => {
		...
		if (userId) {
			...
			const index = meetingInfo.enlistedMembers.findIndex((item: any) => item._id == userId);
